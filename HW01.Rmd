...
Class2 <- select(passenger, survival, class) %>%
  filter(class == 2)
Class2alive <- sum(Class2$survival == 1) 
Class2dead <- sum(Class2$survival == 0) 
Class2total <- length(Class2$survival) 
Class2survivalrate <- Class2alive / Class2total
print (Class2survivalrate)
...
CLASS <- data.frame(Class = c("Class1", "Class2", "Class3"),
  survivalrate = c(Class1survivalrate, Class2survivalrate, Class3survivalrate))
